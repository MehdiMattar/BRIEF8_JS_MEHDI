...
client = Client(host='http://localhost:11434')  # Adjust the host if necessary

# Define a prompt template for asking questions about the CSV
prompt_template = """
...
What is the {column_name} of the row where {condition}?

Provide only the answer without any additional explanation.
"""
...
condition = st.text_input("Enter the condition (e.g., 'year == 2014'):")
...
if st.button("Ask Question"):
    ...
    response = client.generate(model="phi3:mini", prompt=question)
    answer = response['response'].strip()
    st.write(f"Answer: {answer}")

    # Verify the answer
    try:
        verification = df.query(condition)[column_name].values
        st.write(f"Verification from DataFrame: {verification}")
    except:
        st.write("Unable to verify the answer. Please check your condition.")